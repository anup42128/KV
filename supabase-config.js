...
const DatabaseHelper = {
    ...
    async getUnreadMessageCount(userId) {
        try {
            ...
            if (!supabase) {
                console.error('❌ Supabase client not initialized');
                return { success: false, error: 'Database not ready', count: 0 };
            }
    ...
    subscribeToUnreadMessages(userId, callback) {
        try {
            console.log(`📡 Subscribing to unread messages for user: ${userId}`);
            ...
            // Create a unique channel name for this user's unread messages
            const channelName = `unread-messages-${userId}`;
            console.log(`📡 Creating channel: ${channelName}`);
            
            // Create a channel for real-time updates on unread messages
            const channel = supabase
                .channel(channelName)
                .on(
                    'postgres_changes',
                    ...
                    (payload) => {
                        ...
                        if (message.receiver_id === userId && message.is_read === false) {
                            console.log('📥 New unread message for user, calling callback');
                            callback(message);
                        }
                    }
                )
                .on(
                    ...
                    (payload) => {
                        ...
                        // Check if this update affects the current user's unread count
                        const message = payload.new;
                        if (message.receiver_id === userId) {
                            console.log('📥 Message read status changed for user, calling callback');
                            callback(message);
                        }
                    }
                )
                .subscribe((status, err) => {
                    console.log('📡 Unread messages subscription status update:', { status, err });
                    if (status === 'SUBSCRIBED') {
                        console.log('✅ Successfully subscribed to unread messages');
                    } else if (status === 'CHANNEL_ERROR') {
                        console.error('❌ Channel error in unread messages subscription:', err);
                    } else if (status === 'CLOSED') {
                        console.log('🔒 Unread messages subscription channel closed');
                    }
                });

            console.log('✅ Subscribed to unread messages');
            return channel;
        } catch (error) {
    ...
    async getAllUnreadMessageCounts(receiverId) {
        try {
            console.log(`📬 Fetching all unread message counts for user: ${receiverId}`);
            
            if (!supabase) {
                console.error('❌ Supabase client not initialized');
                return { success: false, error: 'Database not ready', counts: {} };
            }
            ...
            return {
                success: false,
                error: 'Failed to load unread message counts',
                counts: {}
            };
        }
    },

    // Get unread message count from a specific sender to current user
    async getUnreadMessageCountFromSender(senderId, receiverId) {
        try {
            console.log(`📬 Fetching unread message count from sender ${senderId} to receiver ${receiverId}`);
            ...
            if (error) {
                console.error('❌ Failed to fetch unread message count from sender:', error);
                return {
                    success: false,
                    error: error.message,
                    count: 0
            ...
            console.log(`✅ Unread message count from sender ${senderId}: ${count}`);
            return {
                success: true,
                count: count || 0
            };
            ...
            console.error('❌ Error fetching unread message count from sender:', error);
            return {
                success: false,
                error: 'Failed to load unread message count',
                count: 0
